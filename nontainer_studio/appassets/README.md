...
One more trap worth recording, because it fails **silently**: the exports
in `scripts/fetch-appassets.sh` are enumerated by hand rather than
`export *`. react and react-dom ship CommonJS, and `export * from` a CJS
module produces no named bindings at all — the first build succeeded,
exported nothing, and the page died with *"does not provide an export
named 'jsx'"*.
